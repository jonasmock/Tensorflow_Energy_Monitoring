...
Then the trained model tries to recognize the number in each image. If the probability is more than 85%, the recognized image is moved to a folder of the respective category, just as it is when classifying. The new pictures can be used in the future for the independent training of the model.
